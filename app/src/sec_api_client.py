...
class SecApiClient:
    def __init__(self, user_agent="CompanyName/YourName (YourEmail)"):
    ...
    def fetch_sec_filings(self, cik, start_days=0, end_days=1):
        ...
        if response.status_code == 200:
            ...
            for date, accession_number, form, primary_document in zip(recent_filings.get('filingDate', []),
                ...
                if datetime.strptime(date, '%Y-%m-%d') > days_ago and form not in ['424B2', '424B5', 'FWP']:
                    ...
                    document_url = self.document_base_url.format(cik=cik, accession_no_slash=accession_no_slash, primary_document=primary_document)
                    self.filings.append({
